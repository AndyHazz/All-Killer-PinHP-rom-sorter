...
mv asterix.zip 'Beat em ups' # Asterix (ver EAD) []
...
mv ga2.zip 'Beat em ups' # Golden Axe: The Revenge of Death Adder (World) [Slow]
...
mv gauntdl.zip 'Beat em ups' # Gauntlet Dark Legacy (version DL 2.52) [Slow]
mv gauntleg.zip 'Beat em ups' # Gauntlet Legends (version 1.6) [Slow]
...
mv kotm2.zip 'Beat em ups' # King of the Monsters 2 - The Next Thing []
...
mv mp_gaxe2.zip 'Beat em ups' # Golden Axe II (Mega Play) []
...
mv bwidow.zip 'Classics' # Black Widow []
mv bzone.zip 'Classics' # Battle Zone (set 1) []
...
mv gaunt22p.zip 'Classics' # Gauntlet II (2 Players, rev 2) []
...
mv invadpt2.zip 'Classics' # Space Invaders Part II (Taito) []
...
mv junofrst.zip 'Classics' # Juno First []
mv kangaroo.zip 'Classics' # Kangaroo []
mv kchamp.zip 'Classics' # Karate Champ (US) []
mv krull.zip 'Classics' # Krull []
mv kungfum.zip 'Classics' # Kung-Fu Master []
...
mv monsterb.zip 'Classics' # Monster Bash []
...
mv ncv1.zip 'Classics' # Namco Classics Collection Vol.1 []
mv ncv2.zip 'Classics' # Namco Classics Collection Vol.2 []
mv neomrdo.zip 'Classics' # Neo Mr. Do! []
...
mv perfrman.zip 'Classics' # Performan (Japan) []
mv phoenix.zip 'Classics' # Phoenix (Amstar) []
mv pisces.zip 'Classics' # Pisces []
...
mv ripoff.zip 'Classics' # Rip Off []
...
mv sdungeon.zip 'Classics' # Space Dungeon []
mv seawolf.zip 'Classics' # Sea Wolf []
mv seawolf2.zip 'Classics' # Sea Wolf II []
...
mv solarq.zip 'Classics' # Solar Quest []
mv spacduel.zip 'Classics' # Space Duel []
mv spacefb.zip 'Classics' # Space Firebird (Nintendo) []
mv starcas.zip 'Classics' # Star Castle (version 3) []
mv stargate.zip 'Classics' # Stargate []
mv starwars.zip 'Classics' # Star Wars (rev 2) []
mv superpac.zip 'Classics' # Super Pac-Man []
mv tapper.zip 'Classics' # Tapper (Budweiser) []
mv timber.zip 'Classics' # Timber []
mv tron.zip 'Classics' # Tron (set 1) []
mv turtles.zip 'Classics' # Turtles []
mv tutankhm.zip 'Classics' # Tutankham []
mv vanguard.zip 'Classics' # Vanguard (SNK) []
mv venture.zip 'Classics' # Venture (version 5 set 1) []
...
mv bjtwin.zip 'Platformers' # Bombjack Twin []
...
mv bublbob2.zip 'Platformers' # Bubble Bobble II (Ver 2.5O 1994/10/05) [Slow]
mv bublbobl.zip 'Platformers' # Bubble Bobble []
mv bubsymph.zip 'Platformers' # Bubble Symphony (Ver 2.5J 1994/10/05) []
...
mv funkyjet.zip 'Platformers' # Funky Jet (World) []
...
mv ldrun2.zip 'Platformers' # Lode Runner II - The Bungeling Strikes Back []
mv ldrun3.zip 'Platformers' # Lode Runner III - The Golden Labyrinth []
mv ldrun4.zip 'Platformers' # Lode Runner IV - Teikoku Karano Dasshutsu []
mv liquidk.zip 'Platformers' # Liquid Kids (World) []
mv liquidku.zip 'Platformers' # Liquid Kids (US) []
mv loderndf.zip 'Platformers' # Lode Runner - The Dig Fight (ver. B) []
mv maglord.zip 'Platformers' # Magician Lord (set 1) []
mv mario.zip 'Platformers' # Mario Bros. (US) []
mv marvland.zip 'Platformers' # Marvel Land (US) []
mv mcatadv.zip 'Platformers' # Magical Cat Adventure []
mv mmaze.zip 'Platformers' # Marchen Maze (Japan) []
mv momoko.zip 'Platformers' # Momoko 120% []
mv moo.zip 'Platformers' # Wild West C.O.W.-Boys of Moo Mesa (ver EA) []
...
mv pc_mtoid.zip 'Platformers' # Metroid (PlayChoice-10) []
mv penbros.zip 'Platformers' # Penguin Brothers (Japan) []
mv pigout.zip 'Platformers' # Pigout []
mv pitfall2.zip 'Platformers' # Pitfall II []
mv psychic5.zip 'Platformers' # Psychic 5 []
...
mv sonson.zip 'Platformers' # Son Son []
...
mv ar_spot.zip 'Puzzle' # Spot (Arcadia) [Slow]
...
mv assaultp.zip 'Run and gun' # Assault Plus (Japan) []
...
mv deadconx.zip 'Run and gun' # Dead Connection (World) []
...
mv flamegun.zip 'Run and gun' # Flame Gunner [Slow]
...
mv nitrobal.zip 'Run and gun' # Nitro Ball (US) []
...
mv slyspy.zip 'Run and gun' # Sly Spy (US revision 3) []
mv smashtv.zip 'Run and gun' # Smash T.V. (rev 8.00) []
...
mv totcarn.zip 'Run and gun' # Total Carnage (rev LA1 03/10/92) []
...
mv aquajack.zip 'Shoot em ups' # Aqua Jack (World) []
mv batrider.zip 'Shoot em ups' # Armed Police Batrider (Japan, version B) []
mv batsugun.zip 'Shoot em ups' # Batsugun (set 1) []
mv battlega.zip 'Shoot em ups' # Battle Garegga (Europe / USA / Japan / Asia) (Sat Feb 3 1996) []
...
mv bloodbro.zip 'Shoot em ups' # Blood Bros. (set 1) []
...
mv cabal.zip 'Shoot em ups' # Cabal (US set 1) []
mv cawing.zip 'Shoot em ups' # Carrier Air Wing (World 901012) []
mv chplft.zip 'Shoot em ups' # Choplifter []
...
mv dariusg.zip 'Shoot em ups' # Darius Gaiden - Silver Hawk (Ver 2.5O 1994/09/19) [Slow]
...
mv drtoppel.zip 'Shoot em ups' # Dr. Toppel's Adventure (World) []
mv dsaber.zip 'Shoot em ups' # Dragon Saber []
...
mv enforce.zip 'Shoot em ups' # Enforce (Japan) []
mv esprade.zip 'Shoot em ups' # ESP Ra.De. (International Ver 1998 4/22) []
mv exedexes.zip 'Shoot em ups' # Exed Exes []
...
mv gnbarich.zip 'Shoot em ups' # Gunbarich []
mv gradius.zip 'Shoot em ups' # Gradius []
mv gradius2.zip 'Shoot em ups' # Gradius II - GOFER no Yabou (Japan New Ver.) []
mv gratia.zip 'Shoot em ups' # Gratia - Second Earth (92047-01 version) []
mv grdius3e.zip 'Shoot em ups' # Gradius III (World ?) []
...
mv imgfight.zip 'Shoot em ups' # Image Fight (Japan) []
mv inthunt.zip 'Shoot em ups' # In The Hunt (World) []
mv kingdmgp.zip 'Shoot em ups' # Kingdom Grandprix (World) []
...
mv metlhawk.zip 'Shoot em ups' # Metal Hawk []
...
mv nightstr.zip 'Shoot em ups' # Night Striker (US) []
...
mv pirates.zip 'Shoot em ups' # Pirates []
...
mv raystorm.zip 'Shoot em ups' # Ray Storm (Ver 2.06A) [Slow]
...
mv rsgun.zip 'Shoot em ups' # Radiant Silvergun (JUET 980523 V1.000) [Slow]
...
mv sokyugrt.zip 'Shoot em ups' # Soukyugurentai / Terra Diver (JUET 960821 V1.000) [Slow]
...
mv srumbler.zip 'Shoot em ups' # The Speed Rumbler (set 1) []
...
mv strkforc.zip 'Shoot em ups' # Strike Force (rev 1 02/25/91) []
...
mv tenkomor.zip 'Shoot em ups' # Tenkomori Shooting (TKM2/VER.A1) [Slow]
...
mv tp84a.zip 'Shoot em ups' # Time Pilot '84 (set 2) []
mv truxton.zip 'Shoot em ups' # Truxton / Tatsujin []
mv truxton2.zip 'Shoot em ups' # Truxton II / Tatsujin II / Tatsujin Oh (Japan) []
...
mv varth.zip 'Shoot em ups' # Varth - Operation Thunderstorm (World 920714) []
mv vasara.zip 'Shoot em ups' # Vasara []
mv vasara2.zip 'Shoot em ups' # Vasara 2 (set 1) []
mv viewpoin.zip 'Shoot em ups' # Viewpoint []
mv vimana.zip 'Shoot em ups' # Vimana []
mv viprp1.zip 'Shoot em ups' # Viper Phase 1 (Japan, New Version) []
mv wndrplnt.zip 'Shoot em ups' # Wonder Planet (Japan) []
mv xevi3dg.zip 'Shoot em ups' # Xevious 3D/G (XV31/VER.A) [Slow]
...
mv cbaj.zip 'Sports' # Cool Boarders Arcade Jam [Slow]
...
mv driftout.zip 'Sports' # Drift Out (Japan) []
...
mv neobombe.zip 'Sports' # Neo Bomberman []
...
mv rallybik.zip 'Sports' # Rally Bike / Dash Yarou []
...
mv spbactn.zip 'Sports' # Super Pinball Action (US) []
...
mv upndown.zip 'Sports' # Up'n Down []
...
mv wg3dh.zip 'Sports' # Wayne Gretzky's 3D Hockey [Slow]
...
mv asurabld.zip 'Vs Fighting' # Asura Blade - Sword of Dynasty (Japan) []
...
mv doapp.zip 'Vs Fighting' # Dead Or Alive ++ (JAPAN) [Slow]
mv ehrgeiz.zip 'Vs Fighting' # Ehrgeiz (EG3/VER.A) [Slow]
mv fatfury3.zip 'Vs Fighting' # Fatal Fury 3 - Road to the Final Victory / Garou Densetsu 3 - haruka-naru tatakai []
mv fgtlayer.zip 'Vs Fighting' # Fighting Layer (FTL0/VER.A) [Slow]
mv garou.zip 'Vs Fighting' # Garou - Mark of the Wolves (set 1) []
mv gaxeduel.zip 'Vs Fighting' # Golden Axe - The Duel (JUETL 950117 V1.000) [Slow]
mv gundamex.zip 'Vs Fighting' # Mobile Suit Gundam EX Revue []
mv jojo.zip 'Vs Fighting' # JoJo's Venture / JoJo no Kimyouna Bouken []
mv jojoba.zip 'Vs Fighting' # JoJo's Bizarre Adventure: Heritage for the Future / JoJo no Kimyouna Bouken: Miraie no Isan []
mv kinst.zip 'Vs Fighting' # Killer Instinct (v1.5d) [Slow]
...
mv lastblad.zip 'Vs Fighting' # Last Blade / Bakumatsu Roman - Gekka no Kenshi, The (set 1) []
mv lastbld2.zip 'Vs Fighting' # Last Blade 2 / Bakumatsu Roman - Dai Ni Maku Gekka no Kenshi, The []
mv macea.zip 'Vs Fighting' # Mace: The Dark Age (HDD 1.0a [CHD]
...
mv mk2.zip 'Vs Fighting' # Mortal Kombat II (rev L3.1) [Slow]
...
mv outfxies.zip 'Vs Fighting' # Outfoxies [Slow]
mv plsmaswd.zip 'Vs Fighting' # Plasma Sword (USA 980316) [Slow]
mv primrage.zip 'Vs Fighting' # Primal Rage (version 2.3) [Slow]
mv rabbit.zip 'Vs Fighting' # Rabbit []
...
mv rvschool.zip 'Vs Fighting' # Rival Schools (USA 971117) [Slow]
...
mv sfex2p.zip 'Vs Fighting' # Street Fighter EX 2 Plus (USA 990611) [Slow]
mv sfexa.zip 'Vs Fighting' # Street Fighter EX (ASIA 961219) [Slow]
...
mv soulclbr.zip 'Vs Fighting' # Soul Calibur (SOC14/VER.C) [Slow]
mv ssf2t.zip 'Vs Fighting' # Super Street Fighter II Turbo (World 940223) []
mv starglad.zip 'Vs Fighting' # Star Gladiator (USA 960627) [Slow]
mv svc.zip 'Vs Fighting' # SvC Chaos - SNK vs Capcom (MVS) []
mv tekken2.zip 'Vs Fighting' # Tekken 2 Ver.B (TES3/VER.B) [Slow]
mv tekken3.zip 'Vs Fighting' # Tekken 3 (TET1/VER.E1) [Slow]
mv tektagt.zip 'Vs Fighting' # Tekken Tag Tournament (TEG3/VER.C1) [Slow]
mv ts2.zip 'Vs Fighting' # Battle Arena Toshinden 2 (USA 951124) [Slow]
mv umk3.zip 'Vs Fighting' # Ultimate Mortal Kombat 3 (rev 1.2) []
mv vf.zip 'Vs Fighting' # Virtua Fighter [Slow]
...
mv xmvsfu.zip 'Vs Fighting' # X-Men Vs. Street Fighter (US 961023) []